...
@Autonomous(name = "LM3 Auto (Pathing Only)", group = "Autonomous")
@SuppressWarnings("FieldCanBeLocal") // Suppress pointless Android Studio warnings
public class LM3AutoPathingOnly extends LinearOpMode {
    // Editable variables
    ...
    private final ElapsedTime simulationTimer = new ElapsedTime(); // Simulation timer for launching
    ...
    public void runOpMode() {
        ...
        while (opModeIsActive()) {
            ...
            if (pathState == State.COMPLETED || runtime.milliseconds() > 40000) {
                break;
            }
    ...
    class StateMachine {
        ...
        public void speedUpLauncher() {
            // Don't do anything here because this is a simulation-only autonomous
        }
                ...
                switch (currentState) {
                    ...
                    case LAUNCH:
                        if (!this.launchCommanded) { // If the launcher hasn't been commanded
                            simulationTimer.reset(); // "Command the launcher"
                            this.launchCommanded = true; // Launch has been commanded
                        }
                        if (simulationTimer.milliseconds() > 2000) { // "If the launch cycle is completed"
                    ...
                    case INTAKE_ARTIFACT_ROW:
                        ...
                        if (lastCommandedPose == robot.poses.PPGArtifacts || lastCommandedPose == robot.poses.PGPArtifacts || lastCommandedPose == robot.poses.GPPArtifacts) {
                            // This is where the intake would be started in a real autonomous
                            ...
                            // This is where the intake would be stopped in a real autonomous
                            nextState();
                        }
                        break;
