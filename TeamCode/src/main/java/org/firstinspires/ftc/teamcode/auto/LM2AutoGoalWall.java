...
import static org.firstinspires.ftc.teamcode.Constants.AprilTagConstants.BLUE_GOAL_TAG_ID;
import static org.firstinspires.ftc.teamcode.Constants.AprilTagConstants.RED_GOAL_TAG_ID;
...
@Autonomous(name="LM3 Autonomous (Backup Goal Wall)", group="Autonomous")
@SuppressWarnings("FieldCanBeLocal") // Suppress pointless Android Studio warnings
public class LM3AutoBackupGoalWall extends LinearOpMode {
    ...
    public void runOpMode() {
        ...
        // Set starting position (goal wall)
        robot.drivetrain.setPose(robot.poses.goalStart);
            ...
            switch (currentState) {
                case START:
                    ...
                    if (runtime.seconds() >= TIME_BEFORE_STARTING) {
                        ...
                        //camera.start(); // Start the camera for AprilTag detection
                    }

                    break;
                case LINING_UP:
                    // Move backwards until a set distance from the AprilTag is reached
                    if (drivetrain.getPose().getX() > -Constants.DISTANCE_FROM_APRIL_TAG) {
                        // Use the Pinpoint to correct for heading error while moving back
                        double forward = -0.2; // Move back at 0.3 power
                ...
                case LAUNCHING:
                    ...
                    break;
                case STRAFING:
                    // Strafe to the right until we are off the launch line
                    if (drivetrain.getPose().getY() > -18) {
        ...
        blackboard.put("autoEndPose", robot.drivetrain.getPose());
    }
}